...
render_plumed_input() {
    ...
    sed \
        -e 's/@RESTART@//' \
        -e 's/@STATE_RFILE@//' \
        -e "s/FILE=DELTAFS/FILE=$deltafs_file/" \
...
if (( ! dry_run )); then
    ...
    check_plumed_action ECV_MULTITHERMAL
    check_plumed_action OPES_EXPANDED
    ...
    rm -f -- "$parse_input" "$parse_deltafs" "$parse_state" "$parse_colvar"
fi
