...
| `build.sh` | BEN(+1) GAFF2/AM1-BCC parameter, ff19SB/OPC topology와 funnel geometry를 만듭니다. |
...
Production은 `work/`에서 1 ns를 실행합니다. `COLVAR`,
...
ff19SB/OPC system. `run.sh` performs minimization, 200 ps heating, 100 ps
equilibration, and one 1 ns Funnel MetaD production run. The
