...
def main() -> int:
    ...
    for atom_number, expected in EXPECTED.items():
        ...
        if observed != expected:
            raise ValueError(
                f"atom {atom_number}: expected {expected}, observed {observed}"
            )
        rows.append((atom_number, *observed))

    output_path.write_text(
        "atom\tresidue\tname\n"
        + "".join(f"{number}\t{residue}\t{name}\n" for number, residue, name in rows),
        encoding="utf-8",
    )
    ...
    print(f"CV atom validation Completed: {output_path}")
    return 0
