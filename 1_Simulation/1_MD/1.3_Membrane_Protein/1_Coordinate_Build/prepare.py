#!/usr/bin/env python3
"""1K4C.pdb1에서 중심을 맞춘 KcsA tetramer와 K+ 7개를 추출한다."""
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="PDB 1K4C에서 tetramer와 filter ion을 추출하고 중심을 맞춥니다."
    )
...
def rewrite_record(
    ...
    """PDB record의 serial, chain과 coordinate를 바꾼다."""
    x, y, z = coordinates

    return (
        f"{line[:6]}{serial:5d}{line[11:21]}{chain}{line[22:30]}"
...
def collect_assembly_records(
    ...
    """Assembly model별 KcsA chain C와 model 1의 filter K+를 선택한다."""
    ...
    return protein_models, potassium_records
...
def validate_assembly(
    ...
    if residue_count != 412 or len(potassium_records) != 7:
        raise SystemExit(
...
def build_output_records(
    ...
    """Model 1–4를 chain A–D로 바꾸고 filter K+를 chain I에 둔다."""
...
def main() -> None:
    ...
    protein_models, potassium_records = collect_assembly_records(input_lines)

    validate_assembly(protein_models, potassium_records)

    center = calculate_center(protein_models, potassium_records)
    output_records = build_output_records(protein_models, potassium_records, center)

    args.output_pdb.parent.mkdir(parents=True, exist_ok=True)
    args.output_pdb.write_text("\n".join(output_records) + "\n", encoding="ascii")

    print(f"KcsA tetramer 전처리 결과: {args.output_pdb} (412 residues, K+ 7개)")
    print(
        "막 생성 전에 protonation을 확인하세요. "
        "E71은 자동으로 결정하지 않습니다."
    )
