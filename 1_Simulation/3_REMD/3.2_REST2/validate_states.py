#!/usr/bin/env python3
"""Validate the replica schedule consumed by the REST2 build script."""
...
HEADER = ["replica", "effective_temperature_K", "lambda_pp", "lambda_pw", "seed"]
...
def validate(path):
    ...
    for row_number, row in enumerate(rows[1:], start=2):
        ...
        replica, temperature_text, lambda_pp_text, lambda_pw_text, seed = row
        ...
        if row_number == 2 and (replica != "000" or abs(temperature - 300.0) > 1.0e-6):
...
def main():
    ...
    parser.add_argument("states", type=Path, help="REST2 states.tsv file")
