...
GaMD, LiGaMD3와 Pep-GaMD의 선택은 leaf directory로 고정되며 별도 config
mode가 아닙니다.
...
LiGaMD3와 Pep-GaMD는 Amber 26의 serial `pmemd.cuda`가 필요합니다. 기본 config의
GaMD parameter-preparation과 production은 NVT이며, 앞선 equilibration은 NPT입니다.
...
GaMD, LiGaMD3, and Pep-GaMD are selected by leaf directory and are not separate
config modes.
...
restart and GaMD state across production segments. By default, GaMD preparation
and production use NVT. Amber 26 LiGaMD3 and Pep-GaMD require serial `pmemd.cuda`.
