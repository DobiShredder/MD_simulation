...
if (( dry_run )); then
    ...
    echo '+ python3 generate_gamd_inputs.py ligamd3 config.toml work/system.parm7 work/inputs'
    exit 0
fi
...
"$python" generate_gamd_inputs.py ligamd3 "$config" "$work_dir/system.parm7" "$work_dir/inputs"
