...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate config-driven AMBER inputs for soluble-protein MD."
    )
...
def resolve(config_path: Path) -> dict[str, object]:
    ...
    values.update(salt_settings(build))
    values.update(run_settings(run))
    return values
...
def write_tleap(output: Path, values: dict[str, object], salt_pairs: int | None) -> None:
    ...
    solvate_command = "solvatebox" if values["box_shape"] == "rectangular" else "solvateoct"
    lines = [
    ...
    lines.extend(disulfide_tleap_lines(values, output.parent / "input.pdb"))
    lines.append(f"{solvate_command} system {water_box} {values['box_distance']:.3f}")
    if salt_pairs is None:
        ...
        lines.extend(
            [
                *salt_tleap_lines(values, salt_pairs),
                "check system",
...
def write_md_inputs(output: Path, values: dict[str, object]) -> None:
    ...
    energy_interval = values["energy_interval"]
    restart_interval = values["restart_interval"]
    ...
    (output / "min-all.in").write_text(
        render_cntrl(
            ...
            ["imin=1", f"maxcyc={values['minimization_steps']}",
             f"ncyc={values['minimization_steepest_steps']}",
             "ntc=1", "ntf=1", "ntb=1", "cut=10.0", "ntr=0"],
        ),
        encoding="utf-8",
    )
    heating_input = render_cntrl(
            ...
            [
                ...
                f"dt={dt_ps:.6f}", f"tempi={values['heating_initial_temperature']:.3f}", f"temp0={temperature:.3f}",
                "ntt=3", "gamma_ln=1.0", f"ig={seed}", "ntb=1", "ntp=0", "ntc=2",
                "ntf=2", "cut=10.0", "ntr=1", "restraint_wt=5.0",
                f"restraintmask='!:WAT,{values['cation']},{values['anion']} & !@H='",
                f"ntpr={energy_interval}", f"ntwx={trajectory_interval}",
                f"ntwr={restart_interval}", "ioutfm=1", "nmropt=1",
            ],
        )
    heating_input += (
        "&wt\n"
        ...
        f"  value1={values['heating_initial_temperature']:.3f},\n"
    ...
    dynamics = [
        ...
        "ntc=2", "ntf=2", "cut=10.0", f"ntpr={energy_interval}",
        f"ntwx={trajectory_interval}", f"ntwr={restart_interval}", "ioutfm=1",
    ]
    (output / "equilibrate.in").write_text(
        render_cntrl(
            ...
            [
                ...
                *amber_ensemble_lines(str(values["equilibration_ensemble"]), float(pressure)),
                "ntr=1",
                "restraint_wt=1.0",
                f"restraintmask='!:WAT,{values['cation']},{values['anion']} & !@H='",
            ],
    ...
    (output / "production.in").write_text(
        render_cntrl(
             ...
             *amber_ensemble_lines(str(values["production_ensemble"]), float(pressure)), "ntr=0"],
        ),
        encoding="utf-8",
    )
...
energy_interval_steps = {values['energy_interval']}
restart_interval_steps = {values['restart_interval']}
random_seed = {seed}
"""
