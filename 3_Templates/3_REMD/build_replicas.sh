...
if (( ! dry_run )); then
    ...
    if ! "$python" -c 'import parmed' >/dev/null 2>&1; then
        die "ParmEd is required. Install 3_Templates/requirements.txt."
    fi
...
if [[ "$method" == rest2 || "$method" == rest3 ]]; then
    ...
    helper_dir="../../../1_Simulation/3_REMD/3.2_REST2"
...
if [[ "$method" == remd ]]; then
    ...
    "$python" ../../../1_Simulation/3_REMD/3.3_REST3/verify_rest3.py \
