...
Build ABFE complex/solvent systems from a reviewed complex PDB and the
precharged ligand parameters and six anchors configured in config.toml.
...
if (( dry_run )); then
    echo "+ $python prepare_fep_systems.py $config $input $build_dir"
...
mkdir -p "$build_dir"
"$python" prepare_fep_systems.py "$config" "$input" "$build_dir"
...
complex_waters=$("$python" count_waters.py "$build_dir/complex.solvated.pdb")
solvent_waters=$("$python" count_waters.py "$build_dir/solvent.solvated.pdb")
