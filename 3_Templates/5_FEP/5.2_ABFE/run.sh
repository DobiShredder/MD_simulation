...
case "$1" in
    ...
    --system|--stage|--windows)
        ...
        ;;
    -h|--help)
        echo "Usage: ./run.sh [--preparation-only | --production-only] [--system complex|solvent] [--stage charge|vdw|restraint] [--windows START-END] [--dry-run]"
        echo
        echo "Run all ABFE restraint, charge, and van der Waals windows."
        echo
        echo "Options:"
        ...
        echo "  --stage NAME         Select charge, vdw, or restraint."
...
if [[ -n "$selected_stage" && "$selected_stage" != charge && "$selected_stage" != vdw && "$selected_stage" != restraint ]]; then
    die "--stage must be charge, vdw, or restraint"
fi
if [[ "$selected_stage" == restraint && "$selected_system" == solvent ]]; then
    die "ABFE restraint windows exist only for the complex system"
fi
...
if [[ -n "$window_range" ]]; then
    ...
    if [[ -z "$maximum_window" || "$window_end" -gt "$maximum_window" ]]; then
        die "Window range exceeds the selected ABFE state range: 0-${maximum_window:-none}"
    fi
fi
if (( ! dry_run )); then
...
if (( dry_run )); then
    ...
    echo "ABFE: $window_count alchemical states, $production_steps production steps per window"
fi
...
if (( selected_state_count == 0 )); then
    die "No ABFE state matches the selected system, stage, and window range"
fi
