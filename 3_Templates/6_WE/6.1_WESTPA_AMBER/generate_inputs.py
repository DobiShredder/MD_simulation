#!/usr/bin/env python3
"""Validate config and generate AMBER inputs for a WESTPA basis state."""
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate AMBER topology-build and basis-state MD inputs."
    )
    parser.add_argument("config", type=Path, help="Template TOML configuration")
    parser.add_argument("output", type=Path, help="Directory for generated AMBER inputs")
    parser.add_argument(
        "--salt-pairs",
        type=int,
...
def write_md_inputs(output: Path, values: dict[str, object]) -> None:
    ...
    (output / "minimize.in").write_text(
        render_cntrl(
            ...
            ["imin=1", "maxcyc=5000", "ncyc=2500", "ntc=1", "ntf=1",
             "ntb=1", "cut=10.0", "ntr=0", f"ntpr={interval}"],
        ),
        encoding="utf-8",
    )
    heating_input = render_cntrl(
            ...
            [
                ...
                "restraintmask='!:WAT,Na+,Cl- & !@H='", f"ntpr={interval}",
                "ntwx=0", f"ntwr={values['heating_steps']}", "nmropt=1",
            ],
        )
    heating_input += (
        "&wt\n"
    ...
    dynamics = [
        "imin=0", "irest=1", "ntx=5", "ntxo=2", f"dt={dt_ps:.6f}",
        ...
        f"ntpr={interval}", "ntwx=0", f"ntwr={values['equilibration_steps']}",
    ]
