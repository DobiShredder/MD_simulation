...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate config-driven AMBER and PLUMED inputs for umbrella sampling."
    )
...
def write_md_inputs(output: Path, values: dict[str, object]) -> None:
    ...
    (output / "min-solvent.in").write_text(
        render_cntrl(
            ...
            [
                ...
                "ntb=1", "cut=10.0", "ntr=1", "restraint_wt=10.0",
    ...
    heating_input = render_cntrl(
            ...
            [
                ...
                f"ntwx={interval}", f"ntwr={interval}", "ioutfm=1", "nmropt=1",
            ],
        )
    heating_input += (
        "&wt\n"
    ...
    (output / "equilibrate.in").write_text(
        render_cntrl(
            "NPT equilibration",
            [f"nstlim={values['equilibration_steps']}", *dynamics, "ntr=1",
             "restraint_wt=1.0", "restraintmask='!:WAT,Na+,Cl- & !@H='"],
        ),
        encoding="utf-8",
    )
